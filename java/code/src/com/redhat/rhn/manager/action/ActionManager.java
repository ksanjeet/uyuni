...
public class ActionManager extends BaseManager {
    ...
    public static PackageAction schedulePackageRunTransaction(User scheduler,
        ...
        for (PackageMetadata pm : pkgs) {
            ...
            } else if (pm.getComparisonAsInt() == PackageMetadata.KEY_THIS_NEWER || pm.getComparisonAsInt() == PackageMetadata.KEY_OTHER_NEWER) {

                if (log.isDebugEnabled()) {
                    log.debug("compare returned [KEY_THIS_NEWER OR KEY_OTHER_NEWER]; " + "deleting package [" + pm.getName() + "-" + pm.getSystemEvr() + "] from system " + "installing package [" + pm.getName() + "-" + pm.getOther().getEvr() + "] to system");
                }
